            ...
            case pygame.KEYDOWN:
                match event.key:
                    case pygame.K_ESCAPE:
                        story.running = False
                    case pygame.K_UP:
                        story.active_item_index = (story.active_item_index - 1) % len(story.menu_items)
                    case pygame.K_DOWN:
                        story.active_item_index = (story.active_item_index + 1) % len(story.menu_items)
                    case pygame.K_RETURN:
                        if (
                            story.menu_items[story.selected_language][story.active_item_index]
                            ...
                            print(f"Selected: {story.menu_items[story.selected_language][story.active_item_index]}")
                            try:
                                last_saved_state: str = SaveAndLoadManager.load_game()
                                story.popup_info = PopupBuilder.init_popup(story.screen,  mode="load_success")
                                ...
                                print(exc.message)
                                story.popup_info = PopupBuilder.init_popup(story.screen, mode="load_failed")
            ...
            case pygame.KEYDOWN:
                match event.key:
                    case pygame.K_ESCAPE:
                        story.running = False
                    case pygame.K_UP:
                        story.active_item_index = (story.active_item_index - 1) % len(story.languages)
                    case pygame.K_DOWN:
                        story.active_item_index = (story.active_item_index + 1) % len(story.languages)
                    case pygame.K_RETURN:
                        story.current_game_state = story.possible_game_states.main_menu
                        story.selected_language = story.languages[story.active_item_index]
