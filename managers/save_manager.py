...
class SaveAndLoadManager:
    ...
    def load_game(save_directory='saved_games') -> str:
        ...
        save_files = [f for f in os.listdir(save_directory) if f.startswith('save_') and f.endswith('.json')]
        if not save_files:
        ...
        latest_save_file = max(save_files, key=lambda f: os.path.getmtime(os.path.join(save_directory, f)))
